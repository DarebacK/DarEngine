...
__declspec(dllimport) int strip_ini_cache (
...
__declspec(dllimport) int load_ini_file (
    FILE * const ini_file,
    const IniFormat format,
    const IniStatsHandler f_init,
    const IniDispHandler f_foreach,
...
__declspec(dllimport) int load_ini_path (
...
__declspec(dllimport) bool ini_string_match_ss (
...
__declspec(dllimport) bool ini_string_match_si (
...
__declspec(dllimport) bool ini_string_match_ii (
...
__declspec(dllimport) bool ini_array_match (
    const char * const ini_string_a,
    const char * const ini_string_b,
...
__declspec(dllimport) size_t ini_unquote (
    char * const ini_string,
    const IniFormat format
);


__declspec(dllimport) size_t ini_string_parse (
    char * const ini_string,
    const IniFormat format
);


__declspec(dllimport) size_t ini_array_get_length (
    const char * const ini_string,
    const char delimiter,
    const IniFormat format
);
...
__declspec(dllimport) int ini_array_foreach (
    const char * const ini_string,
    const char delimiter,
    const IniFormat format,
...
__declspec(dllimport) size_t ini_array_shift (
...
__declspec(dllimport) size_t ini_array_collapse (
    char * const ini_string,
    const char delimiter,
    const IniFormat format
);
...
__declspec(dllimport) char * ini_array_break (
    char * const ini_string,
    const char delimiter,
    const IniFormat format
);
...
__declspec(dllimport) char * ini_array_release (
...
__declspec(dllimport) int ini_array_split (
    char * const ini_string,
    const char delimiter,
    const IniFormat format,
...
__declspec(dllimport) void ini_global_set_lowercase_mode (
...
__declspec(dllimport) void ini_global_set_implicit_value (
    char * const implicit_value,
    const size_t implicit_v_len
);


__declspec(dllimport) IniFormatNum ini_fton (
    const IniFormat format
);


__declspec(dllimport) IniFormat ini_ntof (
    const IniFormatNum format_id
);


__declspec(dllimport) int ini_get_bool (
    const char * const simple_string,
    const int when_fail
);


__declspec(dllimport) int ini_get_bool_i (
    const char * const ini_string,
    const int when_fail,
    const IniFormat format
);
...
extern __declspec(dllimport) int (* const ini_get_int) (
    const char * ini_string
);


extern __declspec(dllimport) long int (* const ini_get_lint) (
    const char * ini_string
);


extern __declspec(dllimport) long long int (* const ini_get_llint) (
    const char * ini_string
);


extern __declspec(dllimport) double (* const ini_get_double) (
    const char * ini_string
);
...
/**
    ...
    @deprecated     Deprecated since version 1.15.0 (it will be removed in
                    version 2.0.0)
**/
__declspec(dllimport) bool INI_GLOBAL_LOWERCASE_MODE;
...
**/
__declspec(dllimport) char * INI_GLOBAL_IMPLICIT_VALUE;


/**
    @brief  Length of the value assigned to implicit keys (default value: `0`)
**/
__declspec(dllimport) size_t INI_GLOBAL_IMPLICIT_V_LEN;
